...
<x-layouts.user :title="$title">
    ...
    <section class="card-surface shadow-auth-card">
        <div class="intake-list-header">
            <div class="min-w-0">
                <h1 class="intake-list-title">{{ __('app.expense.records_list') }}</h1>
                <p class="intake-list-subtitle">{{ $description }}</p>
            </div>

            <a
                href="{{ route('user.expense', $listQuery) }}"
        ...
        >
            <div class="intake-search-grid">
                <div class="intake-search-field intake-search-field-wide">
                    <label for="search-q" class="intake-search-label">{{ __('app.expense.search') }}</label>
                    <div class="intake-search-input-wrap">
                        ...
                        <input
                            ...
                            type="search"
                            value="{{ $filters['q'] }}"
                            placeholder="{{ __('app.expense.search_placeholder') }}"
                ...
                <div class="intake-search-field">
                    <label for="search-from" class="intake-search-label">{{ __('app.expense.release_from') }}</label>
                    <input id="search-from" name="from" type="date" value="{{ $filters['from'] }}" class="intake-search-control">
                </div>

                <div class="intake-search-field">
                    <label for="search-to" class="intake-search-label">{{ __('app.expense.release_to') }}</label>
                    <input id="search-to" name="to" type="date" value="{{ $filters['to'] }}" class="intake-search-control">
                </div>

                <div class="intake-search-field">
                    <label for="search-per-page" class="intake-search-label">{{ __('app.expense.per_page') }}</label>
        ...
        <div id="intake-results" class="intake-results">
            @include('user.pages.partials.expense-results', [
                'expenses' => $expenses,
                'filters' => $filters,
            ])
        </div>
    ...
    <div
        ...
        class="intake-modal"
        aria-hidden="true"
        @if ($errors->any() || filled($editingExpense)) data-open-on-load="true" @endif
    >
        ...
        >
            <div class="intake-modal-header">
                ...
                <div class="min-w-0 flex-1">
                    <h2 id="intake-modal-title" class="text-lg font-bold leading-relaxed text-brand-dark sm:text-xl">
                        {{ filled($editingExpense) ? __('app.expense.edit_title') : __('app.expense.form_title') }}
                    </h2>
                    <p class="mt-1 text-sm leading-relaxed text-brand-muted">
                        @if (filled($editingExpense))
                            {{ __('app.expense.edit_subtitle') }}
                        @else
                            {{ __('app.expense.form_subtitle', ['institute' => __('app.institute')]) }}
                            <span class="mt-1 block font-semibold text-brand-dark">{{ __('app.expense.form_certificate_title') }}</span>
                        @endif
                    </p>
                </div>
                ...
                <a
                    ...
                    aria-label="{{ __('app.expense.close') }}"
                >
            ...
            <div class="intake-modal-body">
                ...
                <div class="intake-form-shell intake-form-shell-modal">
                    @include('user.pages.partials.expense-form', [
