...
<x-layouts.user :title="$title">
    ...
    <section class="card-surface shadow-auth-card">
        <div class="intake-list-header">
            <div class="min-w-0">
                <h1 class="intake-list-title">{{ __('app.assets.records_list') }}</h1>
                <p class="intake-list-subtitle">{{ $description }}</p>
            </div>

            <a
                href="{{ route('user.assets', $listQuery) }}"
        ...
        >
            <div class="intake-search-grid">
                <div class="intake-search-field intake-search-field-wide">
                    <label for="search-q" class="intake-search-label">{{ __('app.assets.search') }}</label>
                ...
                <div class="intake-search-field">
                    <label for="search-from" class="intake-search-label">{{ __('app.assets.registered_from') }}</label>
                    <input id="search-from" name="from" type="date" value="{{ $filters['from'] }}" class="intake-search-control">
                </div>

                <div class="intake-search-field">
                    <label for="search-to" class="intake-search-label">{{ __('app.assets.registered_to') }}</label>
                    <input id="search-to" name="to" type="date" value="{{ $filters['to'] }}" class="intake-search-control">
                </div>

                <div class="intake-search-field">
                    <label for="search-per-page" class="intake-search-label">{{ __('app.assets.per_page') }}</label>
        ...
        <div id="intake-results" class="intake-results">
            @include('user.pages.partials.assets-results', [
    ...
    <div
        ...
        class="intake-modal"
        aria-hidden="true"
        @if ($errors->any() || filled($editingProperty)) data-open-on-load="true" @endif
    >
        ...
        >
            <div class="intake-modal-header">
                ...
                <div class="min-w-0 flex-1">
                    <h2 id="intake-modal-title" class="text-lg font-bold leading-relaxed text-brand-dark sm:text-xl">
                        {{ filled($editingProperty) ? __('app.assets.edit_title') : __('app.assets.form_title') }}
                    </h2>
                    <p class="mt-1 text-sm leading-relaxed text-brand-muted">
                        {{ filled($editingProperty) ? __('app.assets.edit_subtitle') : __('app.assets.form_subtitle') }}
                    </p>
                </div>
                ...
                <a
                    ...
                    aria-label="{{ __('app.assets.close') }}"
                >
            ...
            <div class="intake-modal-body">
                ...
                <div class="intake-form-shell intake-form-shell-modal">
                    @include('user.pages.partials.assets-form', [
