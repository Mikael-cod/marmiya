<x-layouts.user :title="$title">
    <section class="crime-type-page-header overflow-hidden rounded-2xl border border-brand-border bg-brand-surface shadow-auth-card">
        <div class="relative px-4 py-3 sm:px-5">
            ...
            <div class="relative flex flex-row flex-wrap items-center justify-between gap-x-4 gap-y-2">
                ...
                <form method="GET" action="{{ route('user.reports.children-with-mother') }}" class="crime-type-page-filters">
                    ...
                    <a
                        href="{{ route('user.reports.children-with-mother.export', ['eth_year' => $ethYear, 'eth_month' => $ethMonth]) }}"
                        target="_blank"
                        rel="noopener"
    ...
    <section class="card-surface cwm-report-card mt-6 shadow-auth-card">
        <div class="cwm-report-sheet">
            <h2 class="cwm-report-title">{{ $report['title'] }}</h2>
