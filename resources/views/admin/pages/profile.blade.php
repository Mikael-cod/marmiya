<x-layouts.admin :title="$title">
        ...
        <div class="relative px-6 py-8 sm:px-8 sm:py-10">
            ...
            <div class="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-4">
                    <div class="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-brand-teal text-xl font-bold text-white shadow-lg shadow-brand-teal/20">
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    <div class="rounded-2xl border border-brand-border bg-brand-surface px-4 py-3 text-sm">
                        ...
                        <p class="mt-1 font-semibold text-brand-teal">{{ $user->role->label() }}</p>
                    </div>

                    <div class="rounded-2xl border border-brand-border bg-brand-surface px-4 py-3 text-sm">
                        <p class="text-brand-muted">{{ __('app.admin.users.registered_at') }}</p>
                        <p class="mt-1 font-semibold text-brand-dark">{{ $user->created_at?->format('Y-m-d') ?? '—' }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="mt-6 grid gap-6 lg:grid-cols-2">
        <section class="card-surface shadow-auth-card">
            ...
            <form method="POST" action="{{ route('admin.profile.update') }}" class="space-y-5 px-6 py-6">
                @csrf
                @method('PUT')
        ...
        <section class="card-surface shadow-auth-card">
            ...
            <form method="POST" action="{{ route('admin.profile.password') }}" class="space-y-5 px-6 py-6">
                @csrf
                @method('PUT')
